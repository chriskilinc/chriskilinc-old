...
class Navigation extends Component {
  ...
  render() {
    return (
      this.props.navState == "normal-nav" ?
        <nav className="navigation" ref="nav">
          ...
          <NavigationItem isActive={false} id={"contact"} changeActive={this.handleOnNavItemClick} /> */}
            {this.props.navItems.map(item => <NavigationItem key={item.id} isActive={item.isActive} id={item.id} changeToActive={this.props.changeToActive} />)}
          </div>
        </nav>
        :
